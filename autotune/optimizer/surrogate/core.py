...
def build_surrogate(func_str='gp', config_space=None, rng=None, history_hpo_data=None, context=None):
    ...
    if func_str == 'prf':
        try:
            ...
            return RandomForestWithInstances(types=types, bounds=bounds, seed=42, config_space=config_space)
            ...
            return skRandomForestWithInstances(types=types, bounds=bounds, seed=42)
        ...
        return LightGBM(config_space, types=types, bounds=bounds, seed=42)
    elif func_str == 'context_prf':
        from autotune.optimizer.surrogate.base.rf_with_contexts import RandomForestWithContexts
        return  RandomForestWithContexts(types=types, bounds=bounds, seed=42, current_context=context)

    if func_str == 'random_forest':
        from autotune.optimizer.surrogate.skrf import RandomForestSurrogate
        return RandomForestSurrogate(config_space, types=types, bounds=bounds, seed=42)
        ...
        return MFGPE(config_space, history_hpo_data, 42,
